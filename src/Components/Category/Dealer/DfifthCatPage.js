...
const DfifthCatPage = ({ params }) => {
  const searchParams = useSearchParams();
  const id = searchParams.get("id");
  const secID = searchParams.get("secID");
  const { resolvedParamsF } = params || {};
  const { dSecCategory, dThirdCategory, dFourthCategory } =
    resolvedParamsF || {};
  ...
  console.log("dSecCategoryNext", dSecCategory);
  console.log("dThirdCategoryNext", dThirdCategory);
  console.log("dFourthCategoryNext", dFourthCategory);

  useEffect(() => {
    fetchFifthCategories();
  }, [fourthCategoryId, dFourthCategory, dThirdCategory, dSecCategory]);
        ...
        <div className="row justify-content-center categories-list">
          <ul className="subcategories-list d-flex justify-content-center flex-wrap">
            {fifthCategories.map((fifthCategory) => {
              const categoryUrl = `/All/${dSecCategory}/${dThirdCategory}/${dFourthCategory}/${encodeURIComponent(
                fifthCategory.name.replace(/ /g, "-")
              )}?secatEncyt=${encodeURIComponent(
                secID
              )}&secatName=${encodeURIComponent(cSecCategory)}`;
                  ...
                  <div className="item">
                    ...
                    {fifthCategory.sixthCategories &&
                      fifthCategory.sixthCategories.length > 0 && (
                        <Link
                          href={{
                            pathname: `/Dealer/${dSecCategory}/${dThirdCategory}/${dFourthCategory}/${encodeURIComponent(
                              fifthCategory.name.replace(/ /g, "-")
                            )}`,
                            query: {
                              id: fifthCategory.fifthCategoryID,
                              secID: fifthCategory.secondCategoryID,
                            },
                          }}
                          title={fifthCategory.name}
                          style={{ color: "orange" }}
